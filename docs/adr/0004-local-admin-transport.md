...
- 状态：Accepted
- 日期：2026-09-18
- 修订：2026-09-23，明确运行中 Daemon 的 stop/status 经 IPC；原“均在 CLI 内完成”表述被以下决策 1 替代。
...
1. **按运行状态分工**：`daemon start` 与离线 `doctor` 在 CLI 进程内完成；对运行中的 Daemon，`daemon stop|status` 必须经下述本地 IPC，由 Daemon 组合根回答。只有确认没有有效单实例锁时，CLI 才在进程内回答“未运行”。有锁但 IPC 不可达属于错误，不能伪报未运行、直接读写 SQLite 或以强杀进程代替正常停止。具体请求与生命周期以 `LOCAL_ADMIN_PROTOCOL.md` §5.2/§7 为准。
