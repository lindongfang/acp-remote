...
1. **进程内调用为第一优先**：CLI 自己负责的命令（`daemon start|stop|status`、`doctor`、`acp-stdio`）在 CLI 进程内直接完成，不经过任何 IPC。
...
5. **不使用 loopback HTTP 管理面**：`SECURITY_DESIGN.md` §8 的优先级 3 保持未实现状态，除非将来为它单独设计本地认证、Origin/CSRF 与权限模型并新增 ADR。
6. **失败关闭**：IPC endpoint 创建失败（权限不符、路径被占用、socket 被替换为符号链接）时，正式模式拒绝启动而不是降级为"无管理通道"或临时暴露无认证 endpoint。
