...
本文冻结 CLI（`app::cli`）与运行中的 Daemon（`server::local_admin`、`server::acp_facade`）之间本地通道的 endpoint 与访问控制、framing、管理信封、v1 方法集及其 `params`/`result` 形状、本地错误码，以及失败关闭与生命周期规则。
...
- 平台 key 存储与配对密码学（[SECURITY_DESIGN.md](./SECURITY_DESIGN.md) §9、[SYNC_PROTOCOL.md](./SYNC_PROTOCOL.md) §6/§7、[NODE_LINK_PROTOCOL.md](./NODE_LINK_PROTOCOL.md) §9/§13）。
...
- 通道内的 JSON **不是** wire schema：它不与 `schemas/` 下的 Sync/Node Link schema 共享定义，也不进入 `fixtures/`。
...
```text
alias       string    # ^[A-Za-z0-9._-]{1,64}$，发布给 Export 的符号名
...
```text
agentId      string    # ^[A-Za-z0-9._-]{1,64}$
displayName  string    # ≤128
...
required boolean
pattern  string | null                           # 仅 type = "string" 时非 null
enum     (string | integer | boolean)[] | null   # 非空；成员类型必须与 type 一致
```
...
- `params`：`ExportView` 去掉 `createdAt` 与 `revokedAt` 后的全部字段，均为必需；`templates` 与 `workspaceAliases` 的非空与 `default*` 一致性校验不通过 → `local.invalid_params`。
...
- 前置条件：`ownerNodeId` 必须是已配对且 `kind = "owner"` 的节点（否则 `local.not_found`）；`exportIds` 必须存在于该 Owner 的本地 catalog 缓存（否则 `local.not_found`）；`grants` 必须是 `commands.json` 的 `grant.*` 子集且不超过对应 Export 的 `scopes`（违反 `local.invalid_params`）。
...
| `local.not_found` | 目标 `deviceId`/`nodeId`/`pairingId`/`exportId`/`importId` 不存在 | 否 |
