...
| PV1–PV5（RV2 修复轮）/ 3.1–3.8 | RV2 结束后的工作树（提交见 Merge History） | 修复 RV2 的 P1（WP2：终态记录 secret 清除、恒真缓存用例；WP3：R86 死分支）与 P2 后重跑全部证据项 | 主 Agent（coder） | `npm run verify`；`node scripts/check-crate-boundaries.mjs`；`cargo test --locked -p identity-auth --all-features`；`cargo test --locked -p identity-keystore --all-features`；`cargo test … dpapi -- --nocapture`；`cargo clippy --locked -p identity-auth -p identity-keystore --target x86_64-unknown-linux-gnu --all-targets --all-features -- -D warnings`；mutation 检查（见 `reports/rv3-mutation.log`） | Rust 1.98.1 / cargo 1.98.1 / Node v24.19.0；已安装 `x86_64-unknown-linux-gnu` 目标（只能编译/lint，本机无法链接 Linux 二进制） | PASS（identity-auth 78 用例（15 authorization + 19 handshake + 26 pairing + 2 parity + 9 ports + 5 transcripts + 2 doc-tests）、identity-keystore 33 用例（2 lib 单测 + 5 dpapi + 6 entry_format + 8 fail_closed + 12 keystore）；`npm run verify` exit 0；10 crate 边界 OK；Linux 目标 clippy 无告警；10 个 mutation 全部被对应用例捕获） | `reports/rv3-pv1.log`、`reports/rv3-pv2.log`、`reports/rv3-pv3.log`、`reports/rv3-pv4.log`、`reports/rv3-pv5.log`、`reports/rv3-linux-clippy.log`、`reports/rv3-mutation.log`、`reports/rv3-selfcheck-rg.log` |
| 自检（RV1 后补）/ WP2 / 3.3 | 同上 | 计划自检项「`unwrap()`/`expect()`/`from_der` 在非测试代码零命中」 | 主 Agent（coder） | `grep -rn "unwrap()\|expect(\|panic!\|unreachable!\|from_der" crates/identity-auth/src crates/identity-keystore/src` | 同上 | **不符预期，如实记录**（identity-auth 1 处：`types.rs:256` 的 `Digest` 构造不变式；identity-keystore 3 处：`entropy.rs:37-38` 为**测试函数**内的固定断言、`ephemeral.rs:50` 为新建实例不可能中毒）——三处都不是可失败的外部输入路径；原计划措辞「零命中」过强，本轮修正为「可失败路径零 `unwrap`/`expect`」 | `reports/rv3-selfcheck-rg.log` |
...
- **RV1 修复轮新增的检查（2026-09-24）**：
  ...
  2. **可失败性（mutation）检查**：把新增/修正的用例对应的实现逐个退化，断言用例确实失败，再恢复并复跑。理由：RV2 指出「恒真断言」是一类真实风险（见 `reports/rv3-mutation.log`）。证据：`reports/rv3-mutation.log`（10 个 mutation 全部被对应用例捕获）。
...
- **RV2 判定 FAIL（两轮共 4 条 P1）与修复重测（2026-09-24）**：在 `fe52694` 上，三个新的独立 reviewer 子 Agent 分别给出：
  ...
  - 复现与回归证据：`reports/rv3-mutation.log` —— 把对应实现逐个退化后，**10 个 mutation 全部被对应用例捕获**（mutation 1/2：挑战缓存上限与清扫；3/4：`due_pairings` 终态清除与 `complete_auth` 的「已批准」判定；5/6：origin/endpoint 值对象等价比对；7：模式位常量；8/9/9b：标签校验的写入/读取路径；10：R86 两阶段替身），恢复后全部复绿。
