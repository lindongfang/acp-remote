...
- [x] 2.17 [WP3] 负责人：实现 Agent；依赖：2.14；实现内部 `ProcessTree` 抽象与其两个平台实现（`platform/windows` 用 `win32job` 的 `Job::create` + `limit_kill_on_job_close` + `assign_process`，结束用 `TerminateJobObject` 或关闭/丢弃句柄；`platform/unix` 用 `process_group(0)` + `nix::sys::signal::killpg`），并把**平台分支** `cfg` 限制在 `platform.rs` 内；完成条件：两平台均可编译，`rg "cfg\(" crates/agent-host/src` 的**平台分支**只落在 `platform.rs`（`launch.rs` 现有 1 处 `#[cfg(test)]`、`bin/` 零命中，属如实例外）。
...
- [x] 3.6 [WP3] 负责人：独立 reviewer；依赖：3.5，可与 3.5 并行；只读检视子进程所有权与关闭顺序、stdio 分帧与 request id、进程树清理在失败路径也成立、stderr 有界与脱敏、`cfg` 是否只落在 `platform`/`bin` [RV1]；完成条件：报告写入 `reports/rv1-wp3.md`，无未解决阻断项。
