...
- [x] 2.17 [WP3] 负责人：实现 Agent；依赖：2.14；实现内部 `ProcessTree` 抽象与其两个平台实现（`platform/windows` 用 `win32job` 的 `Job::create` + `limit_kill_on_job_close` + `assign_process`，结束用 `TerminateJobObject` 或关闭/丢弃句柄；`platform/unix` 用 `process_group(0)` + `libc::killpg`），并把 `cfg` 限制在该模块与 `bin` 内；完成条件：两平台均可编译，`rg "cfg\(" crates/agent-host/src` 的结果只落在 `platform/**` 与 `bin/**`。
