...
| 7.1 | main（代行独立执行者采集原始输出）/ alternative-verification / final-e2e | **main = e12ac5f**（最终主分支固定版本） | CHECK / 替代验证 4 项（[PV3]、[PV4]、[PV5]、[PV1]+[PV2]） | reports/alt-run-7.1.log（sha256 `305fa6c502a803a32eeca15ebaa0173ce160aa42b54d700c3d78386cc8fce6ec`） | PASS / NEW | 真实执行、逐项显式退出码：`EXIT(pv3-server)=0`、`EXIT(pv4-app)=0`、`EXIT(pv5-vendor)=0`、`EXIT(pv5-server-windows)=0`、`EXIT(npm-run-check)=0`、`EXIT(npm-run-verify)=0`、`EXIT(check-crate-boundaries)=0`；全日志零 `FAILED`；跳过数 0（无 `#[ignore]` 命中；2 个历史 ignored 用例只在 workspace 全量轮出现，日志另有记录） |
| 7.2 | main / aggregation / final-e2e | **main = e12ac5f** | DELIVERY / NOT_APPLICABLE | 本行自身 + reports/alt-run-7.1.log | PASS / NEW | Main E2E = **NOT_APPLICABLE**（理由/依据见 plan.md 的 `### Main E2E`：无跨节点/跨客户端产品闭环、`e2e.command` 为空、降级获批 2026-09-25）；替代验证 4 项全部真实执行且 PASS；覆盖索引的 6 个证据路径全部存在且可读；无未闭环 FAIL/BLOCKED；资源：worktree 仅 1 个、本会话临时目录已清理（2372 → 0），`CARGO_TARGET_DIR` 未覆盖（共用仓库 `target/`） |
...
- 2026-09-25（2.25 交付后，主 Agent 处置 4 项）：
  - **① 关闭顺序措辞冲突 → 主 Agent 已改文档**：实现顺序（停接入 → 排空 → 取消周期任务与信号监听 → 停 Agent → `wal_checkpoint(TRUNCATE)` → 清理并释放锁）与 `SECURITY_DESIGN.md` §12.1 逐字一致；`CORE_PORTS_AND_STORAGE.md` §7.5 第 4 条原措辞「先停周期任务，再停接入层与 Agent」是唯一孤例且会误导后续实现，已改写为「以 §12.1 为准」的显式序列并说明周期任务取消位置的理由（让它们在存储关闭前停止写入，不是先于接入层）。该条措辞的旧版本在 `docs/**`、`README.md`、`AGENTS.md` 内已零命中（`grep -rn 停周期任务` 已确认；任务书与已落盘的检视报告按「历史记录不回改」保留原文）。
...
- 2026-09-25（7.1/7.2，主 Agent）：
  - **[7.1] 替代验证为真实执行**（不以任何检查的 PASS 作为完成条件）：在最终主分支固定版本上跑完 `plan.md` 的 4 项 `alternative_checks`，逐项命令、版本（HEAD）、退出码、日志路径与跳过数见 `reports/alt-run-7.1.log`（**未用 `tail` 截断**，全量输出）。
  - **[7.2] Main E2E 结论：`NOT_APPLICABLE`** —— 理由与依据沿用 `plan.md` 的 `### Main E2E`（`reason`/`basis`/`downgrade_approval`：2026-09-25 用户批准降级），替代验证清单齐备且全部通过；覆盖索引的 6 条证据路径均存在、可读。
  - **资源清理**：worktree 只有 1 个（`D:/Project/acp-remote`）；`CARGO_TARGET_DIR` 未覆盖（共用仓库 `target/`）；`/tmp/acpr-*` 中**本会话产生的 2372 个临时数据目录已清理**（2372 → 0）。**既有残余**：另有约 **14555** 个 `acpr-*` 目录自 2026-09-18 起由 `storage-sqlite` 测试遗留（最早为 `/tmp/acpr-spike`），**早于本变更基线、非本变更引入**，按「不顺手改无关代码」不在本变更内处理，登记为后续独立变更的测试卫生项。
## Dependency Handoffs
...
| RV4-WP5-F2 | cc19ddc | RV4-WP5 | verification.md:138/209（PRO-4 的「隔离重跑 5/5」） | P2：「5/5 通过（各 0.15–0.17s）」无独立日志工件，属自述证据 | **已处理**：在最终候选上重跑 5 次并落盘 `reports/agent-host-flaky-rerun.log`，PRO-4 的 Retest 列已改引该工件（耗时以工件为准） | 已处理 |
| RV4-WP5-F3 | cc19ddc | RV4-WP5 | verification.md:75（「见 6.3 行」悬空） | P2：Handoff Index 无 6.3 行，最终候选的 [PV1]–[PV5] 无归属行 | **已处理**：已补 6.3/6.4 行（`Target Revision` = 最终候选 4595799、证据 = `reports/candidate-verify-final.log` 与 RV2–RV5 报告；sha256 由 premerge 块 pin） | 已处理（随本轮收尾记录落盘） |
...
| PRO-4（候选轮 2a 红；既有 flaky，非本变更回归） | 最终候选 `c050c83` 首跑 [PV1] `EXIT=101`：`agent-host` `supervision.rs:422` `oversize_frame_ends_the_agent_and_fails_pending_requests` panic（断言 `!is_running()` 紧随错误收敛读取进程状态） | 主 Agent（检查执行者） | 同一提交重跑 [PV1] → `EXIT=0`（82 targets / 718 passed / 0 failed / 2 ignored）；该用例隔离重跑在最终候选上重做 5 次全部通过（独立工件 `reports/agent-host-flaky-rerun.log`，5 次耗时 0.35/0.17/0.16/0.18/0.17 s，均 `EXIT(runN)=0`）；红轮原始证据见 `reports/candidate-verify.log` 候选轮 2b 段 | 本变更为 docs + `crates/app` 一行注释的 delta，`git diff main..HEAD -- crates/agent-host` 为空 ⇒ 非本变更引入 | 留待后续变更以「有界等待」修测试侧竞态；本变更内按 §8 不顺手改无关代码 | 已登记；状态 CLOSED（重试绿 + 归因为既有 flaky） |
