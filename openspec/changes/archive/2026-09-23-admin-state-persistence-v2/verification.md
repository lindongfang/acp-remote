...
34. **（新增判据）升级路径与 v2 枚举列的判据补齐**：新增 `a_failed_upgrade_rolls_back_to_v1`（预建占位表 `imported_import_v2` 使第二段升级脚本失败 → `open` 返回错误且库仍是完整 v1：无 `owned_device`/`imported_import_export`、`owned_audit` DDL 无 `provider.configured`、`imported_import` 仍含 `export_id`、`user_version = 1`；去掉注入后重开升级成功）；升级用例补「读视图给出三条事件且 `event_payload` 可还原」（读视图持有读池连接，必须在 `close()` 前 `drop`）与「升级库的 6 张 `imported_*` 表列集合与新建库逐项相等」；升级库的两张审计表 DDL 断言覆盖 `AuditAction::ALL` 每个取值；`enum_coverage.rs` 的 `cases` 由 16 扩到 **26**（v2 管理表 8 个枚举列 + 两张审计表 `actor_kind`）。**残留（不在本变更内，精确描述）**：四列没有进 `enum_coverage.rs` 的 `cases`（26 条）——`owned_device.revoke_reason`、`owned_node.revoke_reason`、`owned_export.cache_policy`、`imported_import.cache_policy`。原因是两类，**不是**「CHECK 不是 `IN (...)` 形状」一概而论：① `revoke_reason` 的 CHECK **是** `IN ('user_requested','key_changed','compromised')`（可被现有解析器识别），缺的是 core 侧取值表——`RevokeReason` 是 `core::ports` 里手写的朴素枚举，**没有** `ALL`/`as_str`，因为它的落库标记按设计留在适配器（`crates/storage-sqlite/src/admin/trust.rs` 的 `revoke_token()`，注释写明「标记只在持久层使用，因此映射留在这里，不给 core 增加一个只被适配器使用的公开方法」）；② `cache_policy` 的 CHECK 是**等值**形状（`= 'no-content-cache'`），而 `enum_coverage.rs` 的 `enum_values()` 只解析 `IN (...)`，所以即便 `CachePolicy::ALL`/`as_str` 已由 `token_enum!` 生成，也无法用现有形状表达。**实际盲区（已核实）**：`RevokeReason::KeyChanged` 在 `revoke_token()` 之外**没有任何用例触及**（`grep -rn KeyChanged crates/` 只命中定义、文档注释与那条映射），也没有任何用例断言这两列拒绝非法取值——即 `key_changed` 的 DDL 文本与映射一旦写错，只有真实 `node.identity_changed` 撤销时的 CHECK 失败才会暴露。**收口配方（不触碰已冻结的 §5 端口面）**：在 `storage-sqlite` 的测试里按适配器口径登记三个 token（与 `revoke_token` 的穷举 `match` 配合——core 新增变体会导致编译失败），补一条 `KeyChanged` 撤销的行为用例，并让 `enum_values()` 支持等值形状以覆盖 `cache_policy`；这三步都在适配器/测试内。
...
| CI-F1 / 6.7 复验 | GitHub Actions `合同门禁 + Rust 检查` @ PR #7 head `7ae63df`(run `35864237199`,attempt 1):`use_cases::tests::create_session_authorizes_before_resolving_the_workspace` FAILED,panic `workspace record: Field`(`crates/core/src/use_cases.rs:1762`) | 主 Agent | 平台相关测试夹具:该用例把 `"Z:\\missing-workspace"` 当绝对路径,而 `WorkspaceRecord::try_new` 的 `Path::is_absolute()` 形状校验在 Unix 上先拒绝它(Windows-only fixture)。改为由 `std::env::temp_dir()` 派生一个不存在的子路径——两个平台上都保证是绝对路径;同时保留注释说明原因 | 同文件既有先例:`use_cases.rs` 的其它用例用 `cfg!(windows)` 分支或 `temp_dir()` 派生路径 | 复测依据:本 PR 的 CI 复跑(同一 `合同门禁 + Rust 检查` job);本地 `cargo test --locked -p core --all-features` 通过 | **修复已落地,待 CI 复测** |
...
**未解决的剩余项**：无未闭环 FAIL/BLOCKED。已登记的实现残留仅一条：`owned_device.revoke_reason`、`owned_node.revoke_reason`、`owned_export.cache_policy`、`imported_import.cache_policy` 四列未进 `enum_coverage.rs` 的枚举覆盖表（`Check Plan Changes` 34 尾段的精确描述与收口配方）；其中 `RevokeReason::KeyChanged` 目前无任何用例触及，其余取值由行为用例覆盖。不影响本次结论。
